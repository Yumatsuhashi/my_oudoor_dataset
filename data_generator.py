...
class DataGenerator:
    ...
    def get_chunk_size_per_data_set(self,get_h_freq=False):
        ...
        # Number of paths in the CIR data
        cir_num_paths = self.params["cir_num_paths"]        
        # Number of receivers and transmitters in the scene
        nb_rx = 1
        nb_tx = len(self.scene.transmitters)

        # rx and tx  arrays number of antennas 
        num_ant_rx = self.scene.rx_array.num_ant
        num_ant_tx = self.scene.tx_array.num_ant
        
        # Chunk size for each data set
        chunk_size_per_data_set = {"timestamps":(1,),
                                    "velocities":(1,3),
                                    "positions": (1,3),                        
                                    "a":(1,nb_rx, num_ant_rx, nb_tx, num_ant_tx, cir_num_paths),
                                    "h_f": (1,nb_rx, num_ant_rx, nb_tx, num_ant_tx, self.params["fft_size"]),            
                                    "tau":(1,nb_rx, nb_tx, cir_num_paths)                                      
                }
        
        # Define the dtype for each data set
        data_types = {
            "timestamps": np.float32,
            "velocities": np.float32,
        ...
        chunk_sizes_per_data_set_bytes = {key: np.prod(chunk_size_per_data_set[key]) * np.dtype(data_types[key]).itemsize for key in data_types.keys()}
        
        if not get_h_freq:
            del chunk_size_per_data_set["h_f"]
            del chunk_sizes_per_data_set_bytes["h_f"]
    ...
    def generate_cir_per_batch(self,trajectories_indices=None, batch_size=10,n_positions_in_parallel=1,file_name=None,get_h_freq=True):
        ...
        if get_h_freq:# Subcarrier frequencies for the OFDM channel response
            self.frequencies = subcarrier_frequencies(self.params["fft_size"], self.params["subcarrier_spacing"])
        ...
        for trajectory_idx in tqdm(trajectories_indices, desc="Processing Trajectories"):
            ...
            try:
                ...
                self._preallocate_datasets(traj_idx=trajectory_idx,max_sizes=max_size_per_data_set,file_name=file_name)
           ...
           # Itteration over the the diffeerent positions of the user in the path by step of batch_size*n_user_in_parallel
            for i in tqdm(range(0, (num_points), batch_size*n_positions_in_parallel_), desc=f"Processing Points in Path {trajectory_idx}"): 
                ...
                for j in range(i, min(i + batch_size*n_positions_in_parallel_, num_points),n_positions_in_parallel_): 
                    ...
                    if get_h_freq:
                        # Size of H_f : [batch size, num_rx, num_rx_ant, num_tx, num_tx_ant, num_time_steps, fft_size]
                        h_f = cir_to_ofdm_channel(self.frequencies, a, tau, normalize=self.params["cir_to_ofdm_normalize"])
                        # H_f : [batch size, num_rx, num_rx_ant, num_tx, num_tx_ant, num_time_steps, fft_size],: tf.complex
                        # H_f : [batch size, num_rx, num_rx_ant, num_tx, num_tx_ant, fft_size]
                        h_f = tf.squeeze(h_f, axis=[5])
                        h_f = tf.transpose(h_f,perm=[1,0,2,3,4,5])
                        # Append the h_f results to the batch list
                        batch_h_f.append(h_f.numpy())

                        
                    
                    # Transpose the data to have the users in the first dimension                    
                    a = tf.squeeze(a, axis=[6])
                    a = tf.transpose(a,perm=(1,0,2,3,4,5))
                    
                    tau = tf.transpose(tau,perm=(1,0,2,3))
                    
                    batch_a.append(a.numpy())
                    batch_tau.append(tau.numpy())
                      

                    # Retrieve positions for the current time steps and intermediate points
                    j_th_positions = self.user_trajectories_dict[trajectory_idx][int(j):int(j + n_positions_in_parallel_)]
                    j_th_time_steps = self.time_steps_dict[trajectory_idx][int(j):int(j + n_positions_in_parallel_)]
                    j_th_velocities = self.user_velocities_dict[trajectory_idx][int(j):int(j + n_positions_in_parallel_)]
                    
                    # Append the results to the batch lists of each data
                    batch_positions.append(j_th_positions)
                    batch_time_steps.append(j_th_time_steps)
                    batch_velocities.append(j_th_velocities)                    
                    
                # Combine batch results // Concatenate the results of the batch over the first dimension
                batch_positions = np.concatenate(batch_positions, axis=0)
                batch_time_steps= np.concatenate(batch_time_steps, axis=0)
                batch_velocities = np.concatenate(batch_velocities, axis=0)
                batch_a = np.concatenate(batch_a, axis=0)
                batch_tau = np.concatenate(batch_tau, axis=0)
                
                # Prepare the batch data to be written to the HDF5 file at the given index
                batch_data = {"timestamps":batch_time_steps,
                ...
                if get_h_freq:
                    batch_h_f = np.concatenate(batch_h_f, axis=0)
                    batch_data["h_f"] = batch_h_f
                
                if self.verbose:
                    print("Batch data shapes:")
                    for k, v in batch_data.items():
                        print('key:',k,"shape:",v.shape,"dtype:",v.dtype)

                # Getting the writing time needed to  see the load of IO operations
                saving_batch_data_time = time.time()
                # Now we write at given indexs
                self._write_to_dataset(traj_index=trajectory_idx,data_dict=batch_data,start_index=i)
                
                if self.verbose:
                    print("Time taken to save the batch data  %.2f seconds" % (saving_batch_data_time))
    ...
    def _preallocate_datasets(self, traj_idx, max_sizes,file_name=None):
        
        # To select the file name where the data will be saved
        if file_name is not None:
            filename = file_name
        else:
            # Get the file name to save the data        
            filename = f"data_trajectories_{'_'.join(map(str, self.trajectories_indices))}.h5"
        
        filepath = os.path.join(self.params["CSI_data_path"], filename)
        ...
        with h5py.File(filepath, 'a',fs_persist=True,libver="latest") as hf:
            ...
            for dataset_name, max_size in max_sizes.items():
                ...
                chunk_size_bytes = self.chunk_sizes_per_data_set_bytes[dataset_name] # Size of one chunk in bytes   
                ...
                if self.verbose:
                    ...
                    print("Chunk size in bytes: ", self.chunk_size_bytes_per_data_set[dataset_name])
...
if __name__ == "__main__":
    ...
    trajectories_meta_info = pd.read_csv("./Scenario/my_paths_random_start_goal_smoothed_meta_data.csv")
    ...
    data_generator.generate_cir_per_batch(trajectories_indices=trajectories_indices, batch_size=10,n_positions_in_parallel=100,file_name=data_set_file_name,get_h_freq=False)
